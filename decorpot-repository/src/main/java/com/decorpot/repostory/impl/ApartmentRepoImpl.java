...
public class ApartmentRepoImpl implements ApartmentRepo {
	...
	private final String getAllApartment = "SELECT aa.aprt_group, aa.apartment_name, aa.bhk, aa.image_id, sum(iga1.IMAGE_PRICE) price from Decorpot.apartment_attribute aa inner join (select * from Decorpot.IMAGE_GROUP_ATTRIBUTE iga inner join Decorpot.SPACES_RANK sr on iga.IMAGE_SPACE = sr.SPACE where iga.group_id >= 0 order by sr.RANK) iga1 on aa.group_id = iga1.group_id group by aa.aprt_group order by aa.apartment_name, aa.bhk, iga1.CREATED_DATE";
	private final String getAllApartmentByName = "SELECT aa.aprt_group, aa.apartment_name, aa.bhk, aa.image_id, sum(iga1.IMAGE_PRICE) price from Decorpot.apartment_attribute aa inner join (select * from Decorpot.IMAGE_GROUP_ATTRIBUTE iga inner join Decorpot.SPACES_RANK sr on iga.IMAGE_SPACE = sr.SPACE where iga.group_id >= 0 order by sr.RANK) iga1 on aa.group_id = iga1.group_id where aa.apartment_name = ? group by aa.aprt_group order by aa.bhk, iga1.CREATED_DATE desc";
	...
	public List<Map<String, Object>> getAllApartmentByName(String apartmentName) {
		return jdbcTemplate.queryForList(getAllApartmentByName, apartmentName);
	}
