...
public class CompressJPEG {
	public static void main(String[] args) throws FileNotFoundException,IOException {
		...
		File file =  new File("../../../imgs_decor/test");
		...
		for (String str : fileNameList) 
		{
			if(str != null)
			{
				File imageFile = new File("../../../imgs_decor/test/"+str);
				
				//Check jpg file
				boolean tmp = imageFile.getName().contains(".jpg");
				System.out.println("is Jpg file = "+tmp);
				
				File compressedFile = new File("../../../imgs_decor/test_medium/"+str);
				if(tmp)
				{
					BufferedImage originalImage = ImageIO.read(imageFile);
					int width = originalImage.getWidth()/2;
					int height =  originalImage.getHeight()/2;
					BufferedImage resizedImage = Scalr.resize(originalImage, 1400,650,Scalr.OP_ANTIALIAS,Scalr.OP_BRIGHTER);// size(originalImage, Scalr.Method.SPEED, Scalr.Mode.FIT_TO_WIDTH,580, 384, Scalr.OP_ANTIALIAS);
